 ...
 * @author Gilberto
 */
public class GerenteSlider extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GerenteSlider</title>");            
            out.println("</head>");
            out.println("<body>");
            ...
            try {
                if (isMultipart) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (item.isFormField()) {
                            ...
                            if (item.getFieldName().equals("ordem_imagem")) {
                                ordem = item.getString();
                            }
                        } else {
                            if (op.equals("alterar")) {
                                imagem = new File(item.getName()).getName();
                                sAltera = sDAO.listaImagemEspecifica(id);
                                imgPattern = sAltera.getImagem();
                ...
                switch (op) {
                    case "alterar":
                        sDAO.alterarImagemSlider(s);
                        break;
                    case "inserir":
                        sDAO.inserirImagemImagemSlider(s);
                }

                response.sendRedirect("admin/lista_imagem_slider.jsp");
            } catch (Exception e) {
                out.println("Não foi possível realizar a ação no slider: " + e.getMessage());
            }
